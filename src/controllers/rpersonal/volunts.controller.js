...
//metodos para los voluntarios del albergue


const getPersonasVol = async (req, res) => {
    try {
        const connection= await getConnection();
        const result = await connection.query("SELECT id_persona, nombre, apellido, edad, grado_academico, telefono, direccion FROM bd_animals.persona WHERE rol = 'Voluntario';");
        res.json(result);
    } catch (error) {
        res.status(500);
...
const getPersonaVol = async (req, res) => {
    try {
        console.log(req.params);
        const { id } = req.params;
        const connection= await getConnection();
        const result = await connection.query("SELECT id_persona, nombre, apellido, edad, grado_academico, telefono, direccion FROM bd_animals.persona WHERE rol = 'Voluntario' AND id_persona = ?", id);
        res.json(result);
    } catch (error) {
        res.status(500);
        res.send(error.message);
...
const addPersonaVol = async (req, res) => {
    try {
        const {rol = "Voluntario", nombre, apellido, edad, grado_academico, telefono, direccion, albergues_id_albergues = 1} = req.body;
...
const updatePersonaVol = async (req, res) => {
    try {
        //const { id } = req.params;
...
const deleteLoginVol = async (req, res) => {
    try {
        console.log(req.params);
        const { id } = req.params;
        const connection= await getConnection();
